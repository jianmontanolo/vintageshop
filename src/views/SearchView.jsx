  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6 animate-fade-in">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{t('search', 'subtitle')}</p>
      </div>

      <div className="relative mb-6 animate-slide-up delay-50">
        <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
        <input className="input pl-9 text-base py-3" placeholder={t('search', 'placeholder')} value={query} onChange={e => setQuery(e.target.value)} autoFocus />
        {query && (
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center text-gray-400 dark:text-gray-500 text-sm animate-fade-in">
        ...
        <div className="card overflow-hidden animate-slide-up">
          <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
            <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">
              {filtered.length} {filtered.length !== 1 ? t('search', 'results') : t('search', 'result')}
            </span>
            <span className="text-sm text-gray-400 dark:text-gray-500 ml-1.5">{t('search', 'for')} "{query}"</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[550px]">
              <thead>
                <tr>
                  <th className="table-th">{t('search', 'estilo')}</th>
                  <th className="table-th">{t('search', 'color')}</th>
                  <th className="table-th">{t('search', 'description')}</th>
                  <th className="table-th">{t('search', 'rack')}</th>
                  <th className="table-th">{t('search', 'date')}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((top, i) => (
                  <tr key={top.id} className="hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors cursor-pointer animate-fade-in" style={{ animationDelay: `${i * 30}ms` }} onClick={() => navigate(`/tops?rack=${top.rackId}`)}>
