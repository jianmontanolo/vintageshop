...
export default function Sidebar({ open, onClose }) {
  ...
  const navItems = [
    ...
    {
      to: '/imprimir-qr', label: t('nav', 'printQR'),
  ...
  return (
    ...
    `}>
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-800">
        <div className="flex items-center justify-center w-9 h-9 bg-brand rounded-lg flex-shrink-0">
          <svg className="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
          </svg>
        </div>
        <div className="min-w-0">
          <p className="font-semibold text-gray-900 dark:text-white text-sm leading-tight">TOP Inventory</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{t('nav', 'management')}</p>
        </div>
        <button className="ml-auto lg:hidden p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800" onClick={onClose}>
      ...
      <div className="border-t border-gray-100 dark:border-gray-800 px-3 py-3 space-y-1">
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          <div className="w-7 h-7 rounded-full bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center flex-shrink-0">
            <span className="text-xs font-semibold text-brand">{user?.email?.[0]?.toUpperCase() ?? 'U'}</span>
          </div>
